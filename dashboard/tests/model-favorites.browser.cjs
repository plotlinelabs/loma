...
(async()=>{
 ...
 assert.deepEqual(catalog.models.filter(m=>m.provider_id==='anthropic').map(m=>m.model_id).sort(),['claude-fable-5-1','claude-opus-5-5']);
 assert.deepEqual(catalog.models.filter(m=>m.provider_id==='codex').map(m=>m.model_id).sort(),['gpt-5.6-sol','gpt-6-astra','gpt-6-luna','gpt-6-sol']);
 assert.ok(catalog.models.some(m=>m.id===catalog.default_model));
