...
(async () => {
  ...
  try {
    ...
    await page.getByRole('dialog', {name:'Tools selection'}).waitFor();
    check('Tools reachable through settings', true);
    await page.keyboard.press('Escape');
    await settings.getByRole('button', { name: /Skills:/ }).click();
    await page.getByRole('dialog', {name:'Skills selection'}).waitFor();
    check('Skills reachable through settings', true);
    await page.keyboard.press('Escape');
