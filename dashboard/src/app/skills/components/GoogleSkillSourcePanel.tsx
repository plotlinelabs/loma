...
export default function GoogleSkillSourcePanel({ source, slug, onUpdated, disabled = false }: { disabled?: boolean; source: GoogleSkillSource; slug: string; onUpdated: () => void }) {
  ...
  async function action(action: string) {
    ...
    try { await skillSourceRequest(`skills/${encodeURIComponent(slug)}/source`, { action }); onUpdated(); }
    catch (e) { setError((e as Error).message); onUpdated(); } finally { setBusy(false); }
  }
  return <section aria-label="Google Docs source" className="border-b px-5 py-3 space-y-2 text-xs">
    <div className="flex flex-wrap items-center gap-2"><span className="font-medium">Google Docs</span><span className="rounded bg-muted px-2 py-1">{source.status.replaceAll("_", " ")}{!source.auto_sync_enabled && " · auto-sync paused"}</span>
