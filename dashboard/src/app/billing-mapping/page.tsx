...
import { fetchActiveContracts, fetchBillingMappings, setBillingAccount, setProductContract, type BillingContract, type BillingOrganization, type BillingStatus } from "@/lib/billing-mapping-api";
...
export default function BillingMappingPage() {
  ...
  async function load() {
    ...
    try { const rows = await fetchBillingMappings(); setOrganizations(rows); setAccountValues(Object.fromEntries(rows.map(o => [o.id, o.monetizeNowAccountId || ""]))); }
  ...
  async function saveAccount(org: BillingOrganization) {
    ...
    try { await setBillingAccount(org.id, accountValues[org.id] || ""); await load(); }
  ...
  const filtered = organizations.filter(o => filter === "all" || o.products.some(p => p.status === filter));
  ...
  return <div className="space-y-4 animate-fade-in-up">
    ...
    <Card className="overflow-hidden"><Table><TableHeader><TableRow><TableHead>Organization</TableHead><TableHead>MonetizeNow Account ID</TableHead><TableHead>Products</TableHead><TableHead>Health</TableHead><TableHead className="w-10"/></TableRow></TableHeader><TableBody>
      {loading ? <TableRow><TableCell colSpan={5} className="h-28 text-center"><RiLoader4Line className="inline animate-spin"/> Loading mappings...</TableCell></TableRow> : filtered.map(org => <Fragment key={org.id}>
        ...
        {expanded === org.id && <TableRow key={`${org.id}-details`}><TableCell colSpan={5} className="bg-muted/30 p-4"><Table><TableHeader><TableRow><TableHead>Product</TableHead><TableHead>Status</TableHead><TableHead>Current contract</TableHead><TableHead>Active contract</TableHead></TableRow></TableHeader><TableBody>{org.products.map(product => <TableRow key={product.id}><TableCell>{product.name}<div className="text-xs text-muted-foreground font-mono">{product.id}</div></TableCell><TableCell><Badge className={colors[product.status]}>{labels[product.status]}</Badge></TableCell><TableCell className="font-mono text-xs">{product.billingId || "Not linked"}{product.contract?.url && <a href={product.contract.url} target="_blank" rel="noreferrer" className="ml-2 text-brand-600"><RiExternalLinkLine size={14} className="inline"/></a>}</TableCell><TableCell>{org.monetizeNowAccountId ? <Select disabled={saving === product.id} value={product.status === "correctly_linked" ? product.billingId : undefined} onValueChange={value => saveContract(org.id, product.id, value)}><SelectTrigger className="w-[280px]"><SelectValue placeholder="Select active contract"/></SelectTrigger><SelectContent>{(contracts[org.id] || []).map(c => <SelectItem key={c.id} value={c.id}>{c.name} ({c.id})</SelectItem>)}</SelectContent></Select> : <span className="text-xs text-muted-foreground">Link the account first</span>}</TableCell></TableRow>)}</TableBody></Table></TableCell></TableRow>}
      </Fragment>)}
      {!loading && !filtered.length && <TableRow><TableCell colSpan={5} className="h-24 text-center text-muted-foreground">No organizations match this filter.</TableCell></TableRow>}
