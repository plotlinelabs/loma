...
type Work = { work_id: string; title: string; agent_id: string; owner: string; instructions: string; success: string; paused: boolean; revoked?: boolean; max_steps: number; max_runtime_minutes?: number; max_planner_retries?: number; policy: Policy; cron?: string; timezone?: string; flow_id?: string; next_run_at?: string; schedule_error?: string };
type Run = { run_id: string; work_id: string; status: string; dry_run: boolean; step: number; max_steps: number; calls_used: number; deadline_at?: string; planner_retries?: number; result: string; question?: string; wake_at: string; parent_id?: string; history: Record<string, unknown>[] };
...
export default function AgentWorkPage() {
  ...
  return <main className="mx-auto h-full w-full max-w-6xl space-y-6 overflow-y-auto p-4 pb-28 md:p-8 md:pb-12">
    ...
    <Dialog open={creating} onOpenChange={v => { if (!busy) setCreating(v); }}><DialogContent className="max-h-[90dvh] overflow-y-auto sm:max-w-2xl"><DialogHeader><DialogTitle>Give an agent a job</DialogTitle><DialogDescription>Start small. Permissions are enforced by a bounded runner, not prompt instructions.</DialogDescription></DialogHeader>
      ...
      <form className="space-y-4" onSubmit={e => {e.preventDefault(); void act(async () => { await api("work", { title, agent_id: agentId, instructions, success, policy: { ...policy, recipients: recipients.split(/[\n,]/).map(s => s.trim()).filter(Boolean) }, delegates, max_steps: budget, max_runtime_minutes: runtime, max_planner_retries: retries }); setCreating(false); setTitle(""); setInstructions(""); setSuccess(""); }, "Work saved paused. Run a safe test, then enable it when ready.");}}>
