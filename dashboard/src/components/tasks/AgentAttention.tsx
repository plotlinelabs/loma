...
export function AgentAttention() {
  ...
  const parts = attention ? [[attention.approvals, "approval"], [attention.questions, "question"], [attention.deliveries, "delivery review"], [attention.charges, "charge review"]].filter(([count]) => Number(count) > 0).map(([count, label]) => `${count} ${label}${count === 1 ? "" : "s"}`).join(" · ") : "";
